...
// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      if (!pair) { setProduct(null); return; }
      setLoading(true);
  ...
  const addToCart = useCallback(() => {
    ...
    try {
      ...
      const items: Array<{ id: string | number; qty?: number } & Record<string, unknown>> = Array.isArray(arr) ? arr.filter(Boolean) : [];
      const existing = items.find((x: { id?: string | number }) => String(x.id) === String(product.id));
      ...
      // salva meta com storeId p/ checkout
      localStorage.setItem("mysnack_cart_meta", JSON.stringify({ storeId: product.storeId, storeName: product.storeName }));
      // Notifica gerenciador e abre o drawer
  ...
  return (
    <main className="pb-24">
      ...
      <div className="px-4 py-6">
        ...
        {!loading && pair && !product && <div className="text-sm text-red-600">Produto não encontrado.</div>}
        {product && (
